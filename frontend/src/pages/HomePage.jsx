...
import { useToast } from '../context/ToastContext.jsx'; // 2. Import useToast

const API_URL = import.meta.env.VITE_API_URL;

// Remember to add your hero image to this path
...
const HomePage = () => {
  ...
  const { showToast } = useToast(); // 4. Get showToast
  ...
  const handleProductDeleted = async (productId) => {
    try {
      ...
      await axios.delete(`${API_URL}/products/${productId}`, {
        // We don't need to send auth headers, axios default has it
      });
      
      // Update the state to remove the product from the list instantly
  ...
  return (
    <div className="container mx-auto space-y-16">
      ...
      <section>
        ...
        {loading && <div className="text-center text-xl dark:text-gray-300">Loading featured products...</div>}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {products.map((product) => (
              <ProductCard 
                key={product._id} 
                product={product} 
                onProductDeleted={handleProductDeleted} // 6. Pass the handler to the card
              />
            ))}
          </div>
        )}
