...
import React, { useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import { useAuth } from "../../context/AuthContext.jsx"; // 1. Added .jsx extension
import { useCart } from "../../context/CartContext.jsx"; // 2. Import useCart

const Header = () => {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const { user, logout } = useAuth();
  const { cart } = useCart(); // 3. Get the cart from context
  const navigate = useNavigate();
  ...
  // We are keeping all of your custom styling (bg-linear-to-r, etc.)
  return (
    <header className="bg-linear-to-r from-gray-900 via-gray-800 to-gray-900 text-white shadow-md sticky top-0 z-50">
      <nav className="container mx-auto px-4 sm:px-6 py-4 flex flex-wrap items-center justify-between">
